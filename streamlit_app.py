...
def display_persona_info(persona_data, conversation):
    """ペルソナ情報を固定表示する関数"""
    ...
    with st_fixed_container(mode="fixed", position="top", border=True, key="persona_info"):
        ...
        if not context_indices:
            st.markdown("このコンテキストに対するペルソナ情報がありません")
            return
        else:
...
def main():
    ...
    # ペルソナ情報の表示
    with persona_col:
        display_persona_info(persona_data, sampled_test_data[page])
